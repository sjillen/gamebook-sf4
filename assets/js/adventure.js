...
const heroSkillElts = document.querySelectorAll('.use-skill');
const heroItemElts = document.querySelectorAll('.use-inventory');
let skillPulse = document.querySelector('.skills-dropup-trigger');
let inventoryPulse = document.querySelector('.inventory-dropup-trigger');
let skillEventCounter = 0;
let itemEventCounter = 0;
...
itemRequirements.forEach( (requirement) => {
    $(requirement).hide();
    itemsRequired.push($(requirement).data('choice-item'));
});
let skillsRequired = [];
skillRequirements.forEach( (requirement) => {
    $(requirement).hide();
    skillsRequired.push($(requirement).data('choice-skill'));
});



// func event : display choice with skill requirement
function displaySkillChoice () {
    //get choice to with corresponding skill
    let choices = [];
    skillRequirements.forEach( (requirement) => {
        let choiceSkill = $(requirement).data('choice-skill');
        if( choiceSkill=== this.innerText) {
            choices.push(requirement);
        }
    });
...
function displayItemChoice () {
    ...
    itemRequirements.forEach( (requirement) => {
        let choiceItem = $(requirement).data('choice-item');
        if(choiceItem === this.innerText) {
    ...
    -- itemEventCounter;
    //if no skill has pulse, remove the pulse from skill bar button
    if (itemEventCounter === 0) {
        removePulse(inventoryPulse);
    }
}
...
//check if item hero matches item of choice
    heroItemElts.forEach( (elt) => {
        ...
        `;
            let buttonContainer = elt.parentNode.parentNode.querySelector('.key-button-icon');
            buttonContainer.innerHTML = keyIcon;
            addIconPulse(elt, "teal");

            //bind event for each active skill :  increment ability
            elt.addEventListener("click", displayItemChoice, false);
            //increment counter for each active skill
            ++ itemEventCounter;
        }
    });
    ...
    if (typeof hasFight !== "undefined") {
        ...
        const isVictory = function () {
            ...
            if (monstersLeft > 0) {
                ...
                let pulsingElts = skillListBtn.querySelectorAll(".pulse");
                pulsingElts.forEach((elt) => {
                    removePulse(elt);
                });
        ...
        const attackFunc = function() {
            ...
            let heroDmg = Math.ceil(7 + nbNpcs - hitDamage);
